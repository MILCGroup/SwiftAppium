...
struct SwiftAppiumCLI: ParsableCommand {
    static let configuration = CommandConfiguration(
        ...
        abstract: "SwiftAppium Web Recorder - Capture browser interactions and generate Swift test code",
        version: "1.0.0"
    )
    ...
    func run() throws {
        ...
        if !isPortAvailable(port: port) {
            print("ERROR: Port \(port) is in use. Please free the port or specify a different one using --port <number>.")
